"""Unit tests for the General governance policy pack.

Validates YAML structure, unique IDs, rule references, applies_to class
references, and required fields for CIS-style benchmark policies.

Run with: pytest tests/unit/test_policy_pack_general.py -v
"""
import pytest
import yaml
...
PACK_DIR = REPO_ROOT / "library" / "general"
...
def class_names(ontology_classes):
    """All class names defined in the general ontology pack."""
    names = set()
...
class TestPolicyContent:
    def test_expected_policy_count(self, policies):
        assert len(policies["policies"]) == 10
    ...
    def test_expected_policy_ids(self, policies):
        expected = {
            "POL-GEN-001", "POL-GEN-002", "POL-GEN-003", "POL-GEN-004", "POL-GEN-005",
            "POL-GEN-006", "POL-GEN-007", "POL-GEN-008", "POL-GEN-009", "POL-GEN-010",
        }
